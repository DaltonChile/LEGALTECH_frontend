...
const WHATSAPP_NUMBER = import.meta.env.VITE_WHATSAPP_NUMBER || '+56945224158';
const WHATSAPP_MESSAGE = import.meta.env.VITE_WHATSAPP_MESSAGE || 'Hola, tengo una consulta sobre ContratoSeguro';

/** Floating WhatsApp button – visible only on public pages */
export function WhatsAppButton() {
  const location = useLocation();
  const [showTooltip, setShowTooltip] = useState(true);

  // Hide on admin and notary routes
  const isAdminOrNotary = location.pathname.startsWith('/admin') || location.pathname.startsWith('/notary');
  if (isAdminOrNotary) return null;
