...
const TABS: { id: TabId; label: string; icon: React.ReactNode }[] = [
  ...
  { id: 'files', label: 'Archivos', icon: <Upload className="w-4 h-4" /> },
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
      ...
      <div className="p-6">
        ...
        <div className="space-y-6">
          ...
          {/* ═══ FILES TAB ═══ */}
          {!showUploader && activeTab === 'files' && activeVersion && (
            <FilesConfigEditor
              templateId={template.id}
          ...
          {!showUploader && activeTab === 'files' && !activeVersion && (
            <div className="text-center py-12 border border-dashed border-slate-300 rounded-md bg-slate-50/50">
              ...
              <p className="font-medium text-slate-500">No hay versión activa</p>
              <p className="text-sm text-slate-400 mt-1">Sube una versión primero para configurar archivos requeridos</p>
            </div>
          )}
