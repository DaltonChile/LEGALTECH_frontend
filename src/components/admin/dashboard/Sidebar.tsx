    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="h-16 flex items-center justify-between px-6 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 bg-gradient-to-br from-navy-900 via-navy-700 to-legal-emerald-700 rounded-lg flex items-center justify-center shadow-lg shadow-navy-900/20">
...
const AccountToggle = () => {
  ...
  return (
    <div className="border-t border-slate-100 p-4">
      <div className="flex items-center gap-3 p-3 rounded-lg bg-slate-50/50 hover:bg-slate-100 transition-colors border border-transparent hover:border-slate-200 cursor-pointer group">
        <div className="w-9 h-9 bg-white text-navy-900 rounded-full flex items-center justify-center font-bold shadow-sm border border-slate-200 group-hover:border-navy-200 group-hover:text-navy-700">
      ...
      {routes.map((route) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 group ${isActive
                ? 'bg-navy-50 text-navy-900'
                : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
              }`}
          >
            <route.icon className={`w-5 h-5 transition-colors ${isActive ? 'text-navy-900' : 'text-slate-400 group-hover:text-slate-600'}`} />
            {route.label}
            {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-legal-emerald-600" />}
          </button>
        );
      })}
    </>
